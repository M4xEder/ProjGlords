...
function alterarQuantidadeLote(nome) {
  ...
  const novo = Number(prompt(`Quantidade nova para o lote "${nome}" (alocados: ${alocados})`, lotesCadastrados[nome].total));
  if (isNaN(novo) || novo < alocados) return alert(`Quantidade inválida! Deve ser >= ${alocados}`);
...
function excluirLote(nome) {
  if (contarGaylordsDoLote(nome) > 0) return alert('Não é possível excluir. Existem gaylords alocadas');
...
function excluirArea(index) {
  const possuiAlocacao = mapaSalvo[index].ruas.some(r => r.posicoes.some(p => p));
...
function excluirRua(aIndex, rIndex) {
  ...
  if (rua.posicoes.some(p => p)) return alert('Não é possível excluir. Existem gaylords alocadas!');
...
function renderMapa() {
  ...
  mapaSalvo.forEach((area, aIndex) => {
    ...
    areaDiv.innerHTML = `<strong>${area.nome}</strong> <button onclick="excluirArea(${aIndex})">Excluir Área</button>`;
...
function confirmarEndereco() {
  ...
  if (!posAtual && alocados >= totalLote) return alert(`Lote "${lote}" já completo`);
...
function contarGaylordsDoLote(nome) {
  let total=0;
  mapaSalvo.forEach(area=>area.ruas.forEach(rua=>rua.posicoes.forEach(p=>{if(p?.lote===nome) total++;})));
  return total;
}
...
function renderDashboard() {
  ...
  Object.entries(lotesCadastrados).forEach(([nome,data])=>{
    ...
    div.innerHTML = `
      ...
      ${usado} / ${data.total}
      <div class="progress-bar"><div class="progress-fill" style="width:${perc}%;background:${data.cor}"></div></div>
...
function expedirLote(nome) {
  ...
  let observacao = alocados.length<totalOriginal ? `Expedido parcialmente: ${alocados.length} de ${totalOriginal}` : 'Expedido completo';
...
function renderExpedidos() {
  ...
  if(historicoExpedidos.length===0){lotesExpedidos.innerHTML='<p>Nenhum lote expedido</p>'; return;}
...
function verDetalhes(index){
  ...
  if(!item.detalhes || item.detalhes.length===0) return alert('Nenhum detalhe');
  let texto = `Detalhes do lote ${item.lote}:\n`;
  item.detalhes.forEach((g,i)=>texto+=`${i+1} - RZ:${g.rz}, Volume:${g.volume}\n`);
  alert(texto);
}
